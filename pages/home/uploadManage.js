  ...
  _GetAsyncStorageBridgeList = async () => {
    try {
      ...
      if (value !== []) {
        ...
        this.state.initBridgeList.map(item => {
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
        <View style={styles.content}>
          ...
          <View style={styles.btnsFilter}>
            <View style={styles.btnFilter1}>
              <Button
                onPress={() => this._btnUploaded()}
                title={'已上传'}
              />
            </View>
            <View style={styles.btnFilter2}>
              <Button
                onPress={() => this._btnUnUpload()}
                title={'未上传'}
              />
            </View>
          </View>
          <View style={styles.btncheck}>
            <TouchableOpacity style={styles.btncheck1} onPress={this._checkedAll.bind(this)}  >
              <CheckBox
          ...
          <FlatList
            ...
            renderItem={({ item, index }) =>
              <TouchableOpacity onPress={this.handleChange.bind(this, index, item)} key={item.QLBM} >
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', height: 60 }}>
                  ...
                  ></CheckBox>
                  <View>
                    <Text style={{ width: 100 }}>{item.QLMC}</Text>
                  </View>
                  <View>
                    <Text style={{ width: 100 }}>{item.uploadStatus == "uploaded" ? '已上传' : '未上传'}</Text>
                  </View>
                  <View>
                    <Text style={{ width: 100 }}>{item.CJSJ}</Text>
                  </View>
                </View>
              </TouchableOpacity>
            }
