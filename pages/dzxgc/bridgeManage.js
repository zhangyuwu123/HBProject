...
export default class BridgeManage extends Component {
  ...
  addQlxx() {
    ...
    fetch('http://demo.d9tec.com/api/app/adddzxqljd', {
      method: 'POST',
      headers: {
  ...
  uploadImage(uri) {
    let formData = new FormData();
    formData.append('Id', 143)
    let file = { uri: uri, type: 'multipart/form-data', name: 'image.jpg' };
    formData.append("file", file);
    fetch('http://demo.d9tec.com/api/app/UploadQlImage?Id=123', {
      method: 'POST',
      headers: {
        'Content-Type': 'multipart/form-data;charset=utf-8',
        Authorization: "Bearer  " + this.state.token
  ...
  _uploadStoreData = async (bridgeList) => {
    try {
      await AsyncStorage.removeItem('dzxgcBridgeList')
      await AsyncStorage.setItem('dzxgcBridgeList', JSON.stringify(bridgeList))
    ...
    return (
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
        <View style={styles.group}>
          ...
          <View style={styles.TextItem1} >
            <Text>完成里程：</Text>
            <Text>{this.state.bridgeInfo.WCLC}</Text>
          </View>
          <View style={styles.separator} />
          <View style={styles.TextItem} >
            <Text>开工时间：</Text>
