...
export default class BridgeManage extends Component {
  ...
  _GetAsyncStorageBridgeList = async () => {
    try {
      const value = await AsyncStorage.getItem('zhfzgcBridgeList');
      if (value !== null) {
  ...
  _uploadStoreData = async (bridgeList) => {
    try {
      await AsyncStorage.removeItem('zhfzgcBridgeList')
      await AsyncStorage.setItem('zhfzgcBridgeList', JSON.stringify(bridgeList))
